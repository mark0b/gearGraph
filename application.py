...
association = db.Table('association',
    db.Column('rack_id', db.Integer, db.ForeignKey('rack.id'),primary_key=True),
    db.Column('gear_id', db.Integer, db.ForeignKey('gear_db.id'),primary_key=True)
)
...
def getData():
    data = db.session.query(gearDb).filter(gearDb.id.in_(list(session['aRack'].keys()))).order_by(gearDb.id).all()
    session['data'] = [{key:item.__dict__[key] for key in item.__dict__.keys() if key != '_sa_instance_state'} for item in data]
    session.modified = True
    return data
...
def index():
    ...
    if session['brand'] == '*ALL*':
        ...
        gear = gearDb.query.filter(gearDb.brand == session['brand']).order_by(gearDb.id).all()
    brands = [item[0] for item in db.session.query(gearDb.brand).distinct(gearDb.brand).all()]
